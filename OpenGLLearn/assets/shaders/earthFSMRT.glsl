...
layout(location = 2) out vec4 outMainTexture;
layout(location = 3) out vec4 outNightTexture;
layout(location = 4) out vec4 outNormalMap;
...
void main()
{
    ...
    outNormalMap = vec4(normalTS,1.0);
    ...
    outMainTexture = mainTexCol;

    vec3 nightTexCol =
        texture(nightTexture, vUV).rgb;
    outNightTexture = vec4(nightTexCol,1.0);

    float specMap =
        texture(specularMap, vUV).r;
    outSpecularMap = vec4(vec3(specMap),1.0);
    ...
    float depth = length(vRay);
    outDepth = vec4(hsb2rgb(vec3(depth / 100,1.0,1.0)),1.0);
